...
url: https://yuanchaofa.com/post/agent-context-management-and-sub-agents
...
Attention 中每个 token 要和所有其他 token 建立 n² 的 pairwise 关系。Context 越长 → 每个 token 的"注意力预算"越少 → 模型可能"忘记"早期的重要指令，或被大量 tool output 稀释关键信息。
...
- **Full vs Compact 表示**：新结果保留完整内容（文件读写的结果），旧结果替换为文件路径引用（压缩的时候）
- 压缩后通过重新读取文件恢复信息
